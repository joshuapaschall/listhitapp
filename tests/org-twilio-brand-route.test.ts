// Auth-gate tests for the org-twilio brand route. No network calls — the
// orchestrator is mocked; we assert the 401/400/403 gates and that a valid
// owner POST delegates to provisionBrand(orgId) once.
...
vi.mock("@/lib/org-twilio/provision-brand", () => ({
  provisionBrand: h.provisionMock,
}))

import { POST } from "../app/api/org-twilio/brand/route"

describe("org-twilio brand route auth", () => {
  beforeEach(() => {
  ...
  test("owner POST provisions once and returns the result", async () => {
    ...
    expect(body).toMatchObject({ ok: true, trustProductSid: "BU123", brandSid: "BN123", brandStatus: "PENDING" })
  })
