...
export default function EmailCampaignReport({ campaign, analytics }: any) {
  ...
  return <Tabs defaultValue="overview" className="space-y-5">
    ...
    <TabsContent value="overview" className="space-y-5">
      <Card className="overflow-hidden border bg-muted/40 dark:bg-muted/40"><CardContent className="grid gap-5 p-6 md:grid-cols-[1fr_auto] md:items-center"><div><p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Primary KPI</p><p className="mt-2 text-sm text-muted-foreground">Open rate</p><p className="text-5xl font-semibold tabular-nums text-foreground md:text-6xl">{pct(r.openRate)}</p><p className="mt-2 text-sm text-muted-foreground">{num(s.uniqueOpens)} unique opens of {num(s.delivered)} delivered</p></div><div className="grid gap-3 sm:grid-cols-2 md:grid-cols-1"><div className="rounded-xl border bg-background p-3 dark:bg-muted/40"><p className="text-xs uppercase tracking-wide text-muted-foreground">Click-to-open</p><p className="mt-1 text-2xl font-semibold tabular-nums text-foreground">{pct(r.clickToOpen)}</p></div><div className="rounded-xl border bg-background p-3 dark:bg-muted/40"><p className="text-xs uppercase tracking-wide text-muted-foreground">CTR</p><p className="mt-1 text-2xl font-semibold tabular-nums text-foreground">{pct(r.ctr)}</p></div></div></CardContent></Card>
    ...
    <TabsContent value="recipients"><Card><CardContent className="pt-6"><CampaignRecipientsTable channel="email" recipients={analytics?.recipients || []} onRowClick={(r) => { setSelected(r); setSheetOpen(true) }} /></CardContent></Card></TabsContent>
    ...
    <RecipientDrilldownSheet open={sheetOpen} onOpenChange={setSheetOpen} campaignId={campaign.id} recipient={selected} />
  </Tabs>
}
