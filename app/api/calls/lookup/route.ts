...
async function buyerNameByPhone(phone: string): Promise<string | null> {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (phoneIsReal && !recent) {
      const name = await buyerNameByPhone(phoneRaw);
    ...
    if (!name && call.from_number) name = await buyerNameByPhone(call.from_number);
