...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  const { data: domain, error } = await supabaseAdmin.from("email_domains").select("*").eq("org_id", orgId).eq("id", params.id).maybeSingle();
  if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
  if (!domain) return NextResponse.json({ ok: false, error: "Domain not found" }, { status: 404 });
  const { data: senders, error: sendersError } = await supabaseAdmin.from("email_senders").select("*").eq("org_id", orgId).eq("domain_id", domain.id);
...
export async function DELETE(_: Request, { params }: { params: { id: string } }) {
  ...
  const { error: deleteError } = await supabaseAdmin.from("email_domains").delete().eq("org_id", orgId).eq("id", params.id);
