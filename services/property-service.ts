...
export class PropertyService {
  // Public site reads: sessionless, so MUST use supabaseAdmin (anon returns 0 rows
  // under RLS). Org-scoped so each tenant site only ever shows its own deals.
  static async getPublishedDeals(orgId: string | null, limit = 6, offset = 0): Promise<DealSummary[]> {
